...
#ifndef __ISPC_STRUCT_cplx__
#define __ISPC_STRUCT_cplx__
struct cplx {
    double re;
    double im;
};
...
#ifndef __ISPC_STRUCT_binding__
#define __ISPC_STRUCT_binding__
struct binding {
    struct cplx z;
    uint32_t color;
};
...
extern "C" {
#endif // __cplusplus
    extern void newtonFrac(uint32_t * buffer, const struct binding * roots, const int32_t n, const uint32_t width, const uint32_t height, const uint32_t max_iterations);
#if defined(__cplusplus) && (! defined(__ISPC_NO_EXTERN_C) || !__ISPC_NO_EXTERN_C )
} /* end extern C */
#endif // __cplusplus
